...
def test_summarize_text(
    ...
    assert (
        result == mock_openai_summary
    ), "The returned summary does not match the expected mock summary"
    ...
    assert (
        call_args["model"] == used_model
    ), f"Expected model {used_model}, but got {call_args['model']}"

    # Check if max_tokens is set correctly (rough estimate)
    assert (
        call_args["max_tokens"] == max_words * 4
    ...
    ), f"Expected first message role to be 'system', but got {messages[0]['role']}"
    assert (
        messages[1]["role"] == "user"
    ), f"Expected second message role to be 'user', but got {messages[1]['role']}"
    assert (
        str(max_words) in messages[0]["content"]
    ), f"Expected {max_words} to be in the system message content"
    assert (
        metadata["title"] in messages[1]["content"]
    ), "Expected video title to be in the user message content"
    assert (
        full_text in messages[1]["content"]
    ), "Expected full transcript text to be in the user message content"
